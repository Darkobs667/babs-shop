...
export function AddToCart({ product }: { product: { id: string; name: string; price: string; stock: number; imageUrl?: string; variants: Variant[] } }) {
  ...
  const handleAdd = () => { if (!available) return; add({ key: `${product.id}:${variant?.id ?? "default"}`, productId: product.id, variantId: variant?.id, productName: product.name, variantName: variant?.name, unitPrice, imageUrl: product.imageUrl }); setAdded(true); window.setTimeout(() => setAdded(false), 1800); };
